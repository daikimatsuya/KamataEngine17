...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Segment segment{ {-0.4f,0.3f,0.0f},{1.0f,0.5f,0.0f} };
	...
	while (Novice::ProcessMessage() == 0) {
		...
		/// ↓更新処理ここから
		///
		if (lpc_.IsCollosion(segment, plane)) {
			color = 0xFF0000FF;
		}
		else {
			color = 0xFFFFFFFF;
		...
		plane.normal = vf_.Normalize(plane.normal);
		...
		dp_.Draw(plane, worldViewProjectionMatrix, viewPortMatrix, WHITE);
